...
/**
 * Validate a SHC raw string
 * @param {string} rawSHC The raw 'shc://' string (from a QR code)
 * @return {boolean} The validation result
 */
const isValidShc = async (rawSHC: string) => {
	return rawSHC.startsWith('shc:/');
}
 ...
 */
function numericShcToJwt(rawSHC: string) {
...
function parseJwtHeader(header: string) {
	const headerData: any = Buffer.from(header, "base64");
